...
"""
    ...
    def _parse_analysis_response(self, response_text: str) -> Dict:
        ...
        # Extract strengths
        strengths_section = re.search(r'STRENGTHS:(.*?)(?=CONCERNS:|FIT:|$)', response_text, re.DOTALL | re.IGNORECASE)
        if strengths_section:
            strengths_text = strengths_section.group(1)
            analysis['strengths'] = [s.strip('- ').strip() for s in re.findall(r'-\\s*(.+)', strengths_text)]
        
        # Extract concerns
        concerns_section = re.search(r'CONCERNS:(.*?)(?=FIT:|RECOMMENDATION:|$)', response_text, re.DOTALL | re.IGNORECASE)
        if concerns_section:
            concerns_text = concerns_section.group(1)
            analysis['concerns'] = [c.strip('- ').strip() for c in re.findall(r'-\\s*(.+)', concerns_text)]
        
        # Extract fit assessment
        fit_match = re.search(r'FIT:\\s*(.+?)(?=RECOMMENDATION:|$)', response_text, re.DOTALL | re.IGNORECASE)
        if fit_match:
            analysis['fit_assessment'] = fit_match.group(1).strip()
        
        # Extract recommendation
        rec_match = re.search(r'RECOMMENDATION:\\s*(.+)', response_text, re.DOTALL | re.IGNORECASE)
        if rec_match:
