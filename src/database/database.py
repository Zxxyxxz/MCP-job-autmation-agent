...
if __name__ == '__main__':
    ...
    def upsert_job(self, job_data):
        ...
        from datetime import datetime
        try:
            existing = self.get_job_by_url(job_data['url'])
            
            if existing:
                self.conn.execute("""
                    UPDATE jobs 
                    SET description = ?, ai_score = ?, analyzed_at = ?
                    WHERE url = ?
                """, (
                    job_data.get('description', ''),
                    job_data.get('ai_analysis', {}).get('score', 0),
                    datetime.now().isoformat(),
                    job_data['url']
                ))
            else:
                self.conn.execute("""
                    INSERT INTO jobs (title, company, location, url, description, ai_score, scraped_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    job_data['title'],
                    job_data['company'],
